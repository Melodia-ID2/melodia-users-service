...
class SearchService:
    ...
    async def index_user(self, user_data: UserSearchIndex):
        endpoint = "/index/users"
        try:
            await self._make_request("POST", endpoint, user_data.dict(exclude_none=True))
            return True
        except Exception:
            return False

    async def delete_user(self, role: str, user_id: UUID):
