...
def get_me(session: Session, user_id: UUID) -> Union[UserProfileResponse, ArtistProfileResponse]:
    ...
    if user_account.role == UserRole.ARTIST:
        photos = users_repo.get_artist_photos(session, user_id)
        links = users_repo.get_artist_links(session, user_id)
...
def get_public_profile(session: Session, user_id: UUID, current_user_id: UUID) -> UserProfilePublic:
    ...
    if account.role == UserRole.ARTIST:
        photos_list = [photo.url for photo in users_repo.get_artist_photos(session, user_id)]
        links_list = [link.url for link in users_repo.get_artist_links(session, user_id)]
...
def update_artist_social_links(session: Session, user_id: UUID, data: SocialLinksUpdateRequest) -> None:
    ...
    users_repo.update_artist_social_links(session, user_id, valid_links)
...
def add_artist_photo(session: Session, user_id: UUID, photo_file_bytes: bytes) -> dict[str, str | int]:
    ...
    current_photos = users_repo.get_artist_photos(session, user_id)
    ...
    try:
        ...
        users_repo.add_artist_photo(session, user_id, uploaded_url, next_position)
...
def delete_artist_photo(session: Session, user_id: UUID, photo_url: str) -> dict[str, str | int]:
    ...
    photo = users_repo.get_artist_photo_by_url(session, user_id, photo_url)
    if not photo:
    ...
    try:
        ...
        users_repo.delete_artist_photo(session, user_id, photo_url)

        # Reordenar las posiciones de las fotos restantes
        remaining_photos = users_repo.get_artist_photos(session, user_id)
        ...
        for photo in photos_to_reorder:
            ...
            users_repo.update_photo_position(session, photo.id, new_position)
...
def reorder_artist_photos(session: Session, user_id: UUID, photo_urls: List[str]) -> dict[str, str | list[str]]:
    ...
    try:
        ...
        for position, url in enumerate(photo_urls, 1):
            users_repo.update_photo_position_by_url(session, user_id, url, position)
